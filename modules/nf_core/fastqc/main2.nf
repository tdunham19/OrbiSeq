process FASTQC {
    ...
    def old_new_pairs = reads instanceof Path || reads.size() == 1 ? [[ input, "${prefix}.${reads.extension}" ]] : reads.withIndex().collect { entry, index -> [ entry, "${prefix}_${index + 1}.${entry.extension}" ] }
